...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]):
    ...
    user = LoginResource.get_user_by_uni(token_data.username)
    if user is None:
        raise credentials_exception
    return user
...
@app.post("/login")
async def login(uni: str = Form(...), password: str = Form(...)):
